...
| 2026-07-16 | Review integral + optimización | Primera carga lenta (imágenes PNG ~4 MB), footer Portal Clientes a `/clientes` (404), cards sin acción, sin SEO técnico, formulario sin funcionar en prod | Ver `docs/review-2026-07-16.md`: WebP + next/image (‑93%), WhatsApp por categoría, emails a info@dacio.com.ar, OG/JSON-LD/robots/sitemap, headers seguridad, rate limit, AGENTS.md + `docs/workflows/` |
| 2026-07-16 | Git en Windows fallaba (pull/push) | `SSL certificate: unable to get local issuer certificate` | `git config http.sslBackend schannel` (queda configurado en el repo) |
